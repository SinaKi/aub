...
\para{Closest Pair kann deterministisch in $O(n \log n)$ Zeit gelöst werden (devide \& conquer). Man kann sogar beweisen, dass Closest Pair vergleichsbasiert nicht Schneller als $\Omega(n \log n)$ gelöst werden kann.}

\paragraph*{Jetzt} Randomisierter Algorithmus, der Closest Pair in erwartet $O(n)$ Zeit löst.

\para{$X$ sei eine Zufallsvariable. Der Erwartungswert einer diskreten Zufallsvariable ist $E(X)=\sum\limits_i \cdot Pr(x=i)$}
...
\para{Wir entwerfen also einen Algorithmus, dessen Laufzeit als Zufallsvariable $X$ dargestellt werden kann. Bei Eingabe $n$ gilt $E(X) = O(n)$.}
...
\para{Falls $\delta_i = \delta_{i+1}$, füge $p_{i+1}$ in Gitter ein. Falls $\delta_1 > \delta_{i+1}$, baue Gitter neu auf für neues $\delta_{i+1}$ (kostet $\Theta(i)$). Schlecht!}

%TODO bild

\para{Bei dieser Konfiguration und Einfügereihenfolge muss nach jedem Schritt das Gitter neu aufgebaut werden. $\Rightarrow$ Laufzeit wieder $\sum\limits_{i=1}^{n-1} \approx n^2$}
